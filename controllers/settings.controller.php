...
// Settings controller: Handles the 'My settings' page. Changes avatar, color, and handles settings forms.
...
function init()
{
	...
	if (!empty($_GET["changeColor"]) and (int)$_GET["changeColor"]
		and $this->esoTalk->validateToken(@$_GET["token"])) $this->changeColor($_GET["changeColor"]);
	
	// Change the user's avatar?
	if (isset($_POST["changeAvatar"])
	 	and $this->esoTalk->validateToken(@$_POST["token"])
		and $this->changeAvatar()) $this->esoTalk->message("changesSaved");
...
function changeAvatar()
{
	...
	switch ($_POST["avatar"]["type"]) {
		...
		case "none":
			...
			if (empty($this->esoTalk->user["avatarFormat"])) return true;
			
			// Delete the avatar and thumbnail files.
			$file = "$avatarFile.{$this->esoTalk->user["avatarFormat"]}";
			if (file_exists($file)) @unlink($file);
			$file = "{$avatarFile}_thumb.{$this->esoTalk->user["avatarFormat"]}";
			if (file_exists($file)) @unlink($file);
