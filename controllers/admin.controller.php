...
function dashboardInit(&$adminController)
{
	...
	$this->serverInfo = array(
 		"esoProjects version" => ESOTALK_VERSION,
...
function settingsInit(&$adminController)
{
	...
	if (isset($_POST["changeLogo"])
		...
		and $this->changeLogo()) $this->esoTalk->message("changesSaved");
...
function saveSettings()
{
	...
	if (empty($_POST["forumTitle"]) or !strlen($_POST["forumTitle"])) {$this->esoTalk->message("avatarError"); return false;}
...
{	
	...
	if (!empty($_GET["q3"]) and $this->esoTalk->validateToken(@$_GET["token"])) $this->changeSkin($_GET["q3"]);
}
