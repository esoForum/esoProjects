...
// Conversation controller: Controls anything to do with the conversation view;
// Posting, editing, pagination, deleting, etc.
...
function init()
{
	...
	if (!$this->conversation) {
		$this->esoTalk->message("cannotViewConversation", false);
		redirect("");
	}
	...
	if ((isset($_POST["saveDraft"]) or isset($_POST["postReply"])) and $this->esoTalk->validateToken(@$_POST["token"])) {
	...
	if (isset($_POST["discardDraft"])) {
		...
		if (!$this->conversation["postCount"] and $this->deleteConversation()) {
			$this->esoTalk->message("conversationDeleted");
			redirect("");
		}
	...
	if ($this->conversation["id"]) {
		...
		if (!empty($_GET["start"])) {
			switch ($_GET["start"]) {
				...
				case "unread":
					...
					$postId = $this->esoTalk->db->result($this->esoTalk->db->query("SELECT postId FROM {$config["tablePrefix"]}posts WHERE conversationId={$this->conversation["id"]} ORDER BY time ASC LIMIT $limit, 1"), 0);
				...
				case "last":
					...
					$postId = $this->esoTalk->db->result($this->esoTalk->db->query("SELECT MAX(postId) FROM {$config["tablePrefix"]}posts WHERE conversationId={$this->conversation["id"]}"), 0);
					redirect($this->conversation["id"], $this->conversation["slug"], "?start=$this->startFrom", "#p$postId");
					break;
		...
		if (isset($_GET["delete"]) and $this->esoTalk->validateToken(@$_GET["token"]) and $this->deleteConversation()) {
			$this->esoTalk->message("conversationDeleted");
			redirect("");
		}
		...
		// Delete a post.
		if (isset($_GET["deletePost"]) and $this->esoTalk->validateToken(@$_GET["token"])) {
			$this->deletePost((int)$_GET["deletePost"]);
			redirect($this->conversation["id"], $this->conversation["slug"], "?start=$this->startFrom");
		}

		// Restore a post.
		if (isset($_GET["restorePost"]) and $this->esoTalk->validateToken(@$_GET["token"])) {
		...
		// Toggle sticky.
		if (isset($_GET["toggleSticky"]) and $this->esoTalk->validateToken(@$_GET["token"])) {
			$this->toggleSticky();
			redirect($this->conversation["id"], $this->conversation["slug"], "?start=$this->startFrom");
		}

		// Toggle locked.
		if (isset($_GET["toggleLock"]) and $this->esoTalk->validateToken(@$_GET["token"])) {
			$this->toggleLock();
			redirect($this->conversation["id"], $this->conversation["slug"], "?start=$this->startFrom");
		}
		
		// Update the conversation title/tags.
		if (isset($_POST["saveTitleTags"]) and $this->esoTalk->validateToken(@$_POST["token"])) {
		...
		if (isset($_POST["saveGroup"]) and $this->esoTalk->validateToken(@$_POST["token"]))
			$this->esoTalk->changeMemberGroup($_POST["member"], $_POST["group"]);
		
		// Save avatarAlignment to the session.
		if (!empty($_POST["avatarAlignment"]) and $this->esoTalk->validateToken(@$_POST["token"]) and in_array($_POST["avatarAlignment"], array("alternate", "right", "left", "none"))) $_SESSION["avatarAlignment"] = $_POST["avatarAlignment"];
		...
		$this->esoTalk->addToBar("right", "<a href='" . makeLink("feed", "conversation", $this->conversation["id"]) . "' id='rss'><span>{$language["RSS"]}</span></a>", 500);
		
		// Add the sticky/unsticky link if the user has permission.
		if ($this->canSticky() === true) $this->esoTalk->addToBar("right", "<a href='" . makeLink($this->conversation["id"], $this->conversation["slug"], "?toggleSticky", $this->startFrom ? "&start=$this->startFrom" : "", "&token={$_SESSION["token"]}") . "' onclick='Conversation.toggleSticky();return false' id='stickyLink'>" . $language[in_array("sticky", $this->conversation["labels"]) ? "Unsticky" : "Sticky"] . "</a>", 400);
		
		// Add the lock/unlock link if the user has permission.
		if ($this->canLock() === true) $this->esoTalk->addToBar("right", "<a href='" . makeLink($this->conversation["id"], $this->conversation["slug"], "?toggleLock", $this->startFrom ? "&start=$this->startFrom" : "", "&token={$_SESSION["token"]}") . "' onclick='Conversation.toggleLock();return false' id='lockLink'>" . $language[$this->conversation["locked"] ? "Unlock" : "Lock"] . "</a>", 300);
		
		// Add the delete conversation link if the user has permission.
		if ($this->canDeleteConversation() === true) $this->esoTalk->addToBar("right", "<a href='" . makeLink($this->conversation["id"], $this->conversation["slug"], "?delete", "&token={$_SESSION["token"]}") . "' onclick='return Conversation.deleteConversation()'>{$language["Delete conversation"]}</a>", 200);
		
		// Update the user's last action.
		$this->updateLastAction();
		
		// Add the meta tags (description + keywords) to the head.
		$description = $this->esoTalk->db->result("SELECT LEFT(content, 256) FROM {$config["tablePrefix"]}posts WHERE conversationId={$this->conversation["id"]} ORDER BY time ASC LIMIT 1", 0);
		...
		$this->esoTalk->addToHead("<meta property='og:description' content='$description'/>");
		$this->esoTalk->addToHead("<meta name='twitter:description' content='$description'/>");
	...
	// If this conversation doesn't exist (it's a new conversation)...
		...
		$this->esoTalk->updateLastAction($language["Starting a conversation"]);
	...
	if (isset($_GET["removeMember"]) and $this->esoTalk->validateToken(@$_GET["token"])) {
...
function ajax()
{
	...
	switch (@$_POST["action"]) {
		...
		case "changeMemberGroup":
			if (!$this->esoTalk->validateToken(@$_POST["token"])) return;
			$this->esoTalk->changeMemberGroup(@$_POST["memberId"], @$_POST["group"]);
			break;
...
function getConversation($id = false)
{
	...
	if ($id !== false) {
		...
		$from = array(
			...
			"LEFT JOIN {$config["tablePrefix"]}status s ON (s.conversationId=c.conversationId AND s.memberId=" . ($this->esoTalk->user["memberId"] ? $this->esoTalk->user["memberId"] : 0) . ")",
			"INNER JOIN {$config["tablePrefix"]}members sm ON (sm.memberId=c.startMember)"
		);

		$where = array("c.conversationId=$id");
		if (!$this->esoTalk->user) $where[] = "c.posts>0 AND c.private=0";
		else $where[] = "c.startMember={$this->esoTalk->user["memberId"]} OR (c.posts>0 AND (c.private=0 OR s.allowed OR (SELECT allowed FROM {$config["tablePrefix"]}status WHERE conversationId=c.conversationId AND memberId='{$this->esoTalk->user["account"]}')))";
...
function &getMembersAllowed()
{
	...
	// Otherwise, fetch the members allowed from the database.
		...
		$result = $this->esoTalk->db->query("SELECT s.memberId, IF(m.name IS NOT NULL,m.name,s.memberId) FROM {$config["tablePrefix"]}status s LEFT JOIN {$config["tablePrefix"]}members m USING (memberId) WHERE s.conversationId={$this->conversation["id"]} AND s.allowed=1");
...
function getPosts($criteria = array(), $display = false)
{
	...
	while ($post = $this->esoTalk->db->fetchAssoc($result)) {
...
function addReply($content, $newConversation = false)
{
	...
	$post = array(
		...
		"memberId" => $this->esoTalk->user["memberId"],
		"time" => $time,
	...
	$id = $this->esoTalk->db->lastInsertId();
	...
	if (!$newConversation) {
		...
		$this->esoTalk->db->query("UPDATE {$config["tablePrefix"]}conversations SET startTime=IF(posts IS NULL OR posts=0,$time,startTime), posts=IF(posts IS NOT NULL,posts+1,1), lastPostTime=$time, lastActionTime=$time, lastPostMember={$this->esoTalk->user["memberId"]} WHERE conversationId={$this->conversation["id"]}");
...
function discardDraft()
{
	...
	$query = "UPDATE {$config["tablePrefix"]}status SET draft=NULL WHERE conversationId={$this->conversation["id"]} AND memberId={$this->esoTalk->user["memberId"]}";
...
function deletePost($postId)
{
	...
	if (!$this->esoTalk->user) {
		$this->esoTalk->message("noPermission");
		return false;
	}
	// Is the user suspended?
	if ($this->esoTalk->isSuspended()) {
		$this->esoTalk->message("suspended");
		return false;
	}
	...
	$query = "UPDATE {$config["tablePrefix"]}posts p, {$config["tablePrefix"]}conversations c
		...
		WHERE postId=$postId AND c.conversationId=p.conversationId" . ($this->esoTalk->user["moderator"] ? "" : " AND p.memberId={$this->esoTalk->user["memberId"]}");	
	$this->esoTalk->db->query($query);
	
	// If the query didn't affect any rows, either we didn't have permission or the post didn't exist...
	if (!$this->esoTalk->db->affectedRows()) {
		$this->esoTalk->message("noPermission");
		return false;
	}
...
function restorePost($postId)
{
	...
	$postId = (int)$postId;
	$time = time();
	$this->esoTalk->db->query("UPDATE {$config["tablePrefix"]}posts p, {$config["tablePrefix"]}conversations c
...
function startConversation($conversation)
{
	...
	$insert = array(
		...
		"startTime" => $time,
		"lastPostTime" => $time,
		"lastPostMember" => $this->esoTalk->user["memberId"],
		"lastActionTime" => $time,
	...
	$this->esoTalk->db->query($query);
	$conversationId = $this->esoTalk->db->lastInsertId();
	...
	$this->conversation["startMember"] = $this->esoTalk->user["memberId"];
	$this->conversation["startMemberName"] = $this->esoTalk->user["name"];
	...
	if (!($conversation["draft"] ? $this->saveDraft($conversation["content"]) : $this->addReply($conversation["content"], true))) {
		$this->esoTalk->db->query("DELETE FROM {$config["tablePrefix"]}conversations WHERE conversationId=$conversationId");
	...
	if ($conversation["starred"]) {
		$query = "INSERT INTO {$config["tablePrefix"]}status (conversationId, memberId, starred) VALUES ({$this->conversation["id"]}, {$this->esoTalk->user["memberId"]}, 1) ON DUPLICATE KEY UPDATE starred=1";
		$this->esoTalk->db->query($query);
	}
...
function saveTitle($title)
{
	...
	$this->esoTalk->db->query("UPDATE {$config["tablePrefix"]}posts p SET title='$slashedTitle' WHERE conversationId={$this->conversation["id"]}");
}
...
{
	global $language;
	$this->esoTalk->updateLastAction("{$language["Viewing"]} " . (($this->conversation["private"] or $this->conversation["postCount"] == 0)
...
{
	return $this->esoTalk->user and ((!$this->conversation["locked"] and $this->conversation["startMember"] == $this->esoTalk->user["memberId"]) or $this->esoTalk->user["moderator"]);
}

// To edit members allowed, user must be: conversation starter
function canEditMembersAllowed()
{
	return $this->esoTalk->user and $this->esoTalk->user["memberId"] == $this->conversation["startMember"];
}
...
function canReply()
{
	if (!$this->esoTalk->user) return "loginRequired";
	if ($this->esoTalk->isSuspended()) return "suspended";
	if ($this->conversation["locked"] and !$this->esoTalk->user["moderator"]) return "locked";
	return true;
}
...
function canEditPost($postId, $memberId = false, $account = false, $deleteMember = -1)
{
	global $config;
	$postId = (int)$postId;
	if (!$memberId or !$account or $deleteMember === -1) list($memberId, $account, $deleteMember) = $this->esoTalk->db->fetchRow("SELECT p.memberId, account, deleteMember FROM {$config["tablePrefix"]}posts p INNER JOIN {$config["tablePrefix"]}members USING (memberId) WHERE postId=$postId");
	
	if (!$this->esoTalk->user or (!$this->esoTalk->user["moderator"] and ($memberId != $this->esoTalk->user["memberId"] or ($deleteMember and $deleteMember != $this->esoTalk->user["memberId"]))) or ($account == "Administrator" and !$this->esoTalk->user["admin"])) return "noPermission";
	if ($this->conversation["locked"] and !$this->esoTalk->user["moderator"]) return "locked";
	if ($this->esoTalk->isSuspended()) return "suspended";
	return true;
}
...
{
	global $config;
	return $this->esoTalk->user and ($this->esoTalk->user["moderator"] or ($this->esoTalk->user["memberId"] == $this->conversation["startMember"] and ($this->conversation["postCount"] <= 1 or !$this->esoTalk->db->result("SELECT 1 FROM {$config["tablePrefix"]}posts WHERE conversationId={$this->conversation["id"]} AND memberId!={$this->esoTalk->user["memberId"]}", 0))));
}
