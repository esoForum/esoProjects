...
function toggleEnabled(id, enabled) {
	Ajax.request({
		...
		"success": function() {
			if (this.messages) document.getElementById("plugin-" + id + "-checkbox").checked = !enabled;
			else document.getElementById("plugin-" + id).className = "plugin" + (enabled ? " enabled" : "");
		}
	});
}
...
function toggleSettings(id) {
	for (var i in plugins) {
		if (plugins[i] != id && document.getElementById("plugin-" + plugins[i] + "-settings") && document.getElementById("plugin-" + plugins[i] + "-settings").showing)
			hide(document.getElementById("plugin-" + plugins[i] + "-settings"), {animation: "verticalSlide"});
	}
	toggle(document.getElementById("plugin-" + id + "-settings"), {animation: "verticalSlide"});
}
var plugins = [];
// ]]>
</script>
...
<?php if(!empty($plugin["settings"])):if(@$_POST["plugin"]!=$k):?>hide(document.getElementById("plugin-<?php echo $k;?>-settings"));<?php
 else:?>document.getElementById("plugin-<?php echo $k;?>-settings").showing = true;<?php endif;endif;?> 
// ]]></script>
</li>
<?php endforeach;?>
</ul>
...
<?php echo $this->esoTalk->htmlMessage("downloadPlugins","https://esotalk.net/plugins");?>
