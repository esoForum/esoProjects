...
<?php
if(!$this->conversation["id"] and ($error=$this->canStartConversation())!==true):echo $this->esoTalk->htmlMessage($error);else:if(!$this->conversation["id"]):?><form id='sconvo' action='<?php echo curLink();?>' method='post' enctype='multipart/form-data'><?php endif;?>
...
<?php
if($this->conversation["postCount"]>0){$handleWidth=max($config["postsPerPage"]/$this->conversation["postCount"],0.2)*100;if($this->conversation["postCount"]<=$config["postsPerPage"])$percentPerPost=100/$this->conversation["postCount"];else $percentPerPost=$this->conversation["postCount"]?(100-$handleWidth)/($this->conversation["postCount"]-$config["postsPerPage"]):0;$showingPercent=max($percentPerPost*min($this->conversation["postCount"]-$this->startFrom,$config["postsPerPage"]),20);$leftPercent=max(0,min(100-$showingPercent,$this->startFrom*$percentPerPost));$rightPercent=100-($showingPercent+$leftPercent);$unreadLeft=($this->esoTalk->user and $this->conversation["id"])?min($this->conversation["postCount"],$this->conversation["lastRead"])*(100/$this->conversation["postCount"]):100;$unreadWidth=max(0,100-$unreadLeft);}else $percentPerPost=$unreadLeft=$showingPercent=$leftPercent=$rightPercent=$unreadWidth=0;$previousButton=$this->startFrom<=0?"<a class='previous disabled'>{$language["Previous"]}</a>":"<a href='".makeLink($this->conversation["id"],$this->conversation["slug"],"?start=".max(0,$this->startFrom-$config["postsPerPage"]))."' class='previous'>{$language["Previous"]}</a>";$firstButton="<a href='".makeLink($this->conversation["id"],$this->conversation["slug"])."' class='first'>{$language["First"]}</a>";$lastButton="<a href='".makeLink($this->conversation["id"],$this->conversation["slug"],"?start=last")."' class='last'>{$language["Last"]}</a>";$nextButton=$this->startFrom+min($config["postsPerPage"],$this->conversation["postCount"]-$this->startFrom)>=$this->conversation["postCount"]?"<a class='next disabled'>{$language["Next"]}</a>":"<a href='".makeLink($this->conversation["id"],$this->conversation["slug"],"?start=".min($this->conversation["postCount"],$this->startFrom+$config["postsPerPage"]))."' class='next'>{$language["Next"]}</a>";$viewing=sprintf($language["viewingPosts"],"<span class='pgFrom'>".($this->startFrom+1)."</span>","<span class='pgTo'>".($this->startFrom+min($config["postsPerPage"],$this->conversation["postCount"]-$this->startFrom))."</span>","<span class='pgCount'>{$this->conversation["postCount"]}</span>");$paginationHtml="<ul id='pagination' class='pg'>
...
</ul>";$this->callHook("beforeRenderPagination",array(&$paginationHtml));echo $paginationHtml;?>
...
<?php
if(!empty($this->conversation["posts"])):$side=false;$alternateAvatars=false;switch(!empty($this->esoTalk->user["avatarAlignment"])?$this->esoTalk->user["avatarAlignment"]:$_SESSION["avatarAlignment"]){case "right":$side="r";break;case "left":$side="l";break;case "none":break;default:$side=$this->startFrom%2?"l":"r";$alternateAvatars=true;}foreach($this->conversation["posts"] as $k=>$post):$singlePost=false;if(!empty($post["deleteMember"])):?>
...
<?php continue;endif;if(!isset($this->conversation["posts"][$k-1]["memberId"]) or $this->conversation["posts"][$k-1]["memberId"]!=$post["memberId"] or !empty($this->conversation["posts"][$k-1]["deleteMember"])):?>
...
<?php
if($alternateAvatars and isset($this->conversation["posts"][$k+1]["memberId"]) and empty($this->conversation["posts"][$k+1]["deleteMember"]))$side=$side=="r"?"l":"r";endif;endforeach;endif;?>
</div>
<hr/>
<?php
...
<?php
if(($error=$this->canReply())!==true):echo $this->esoTalk->htmlMessage($error);else:?>
